...
class ResNetv2ConvLayer(nn.Module):
    # Copied from transformers.models.resnet.modeling_resnet.ResNetConvLayer.__init__ with ResNet->ResNetv2
    def __init__(
    ...
    def forward(self, input: Tensor) -> Tensor:
        hidden_state = self.normalization(input)
        hidden_state = self.activation(hidden_state)
        hidden_state = self.convolution(hidden_state)
        return hidden_state


# Copied from transformers.models.resnet.modeling_resnet.ResNetEmbeddings with ResNet->ResNetv2
class ResNetv2Embeddings(nn.Module):
    ...
    def __init__(self, config: ResNetv2Config):
        super().__init__()
        self.embedder = ResNetv2ConvLayer(
...
# Copied from transformers.models.resnet.modeling_resnet.ResNetShortCut with ResNet->ResNetv2
...
class ResNetv2PreActBottleneck(nn.Module):
    """
    Pre-activation (v2) bottleneck block. Follows the implementation of "Identity Mappings in Deep Residual Networks":
    https://github.com/KaimingHe/resnet-1k-layers/blob/master/resnet-pre-act.lua Except it puts the stride on 3x3 conv
    when available.
    """
    ...
    def __init__(
        ...
        self.layer = nn.Sequential(
            ...
            ResNetv2ConvLayer(reduces_channels, out_channels, kernel_size=1),
        )
    ...
    def forward(self, hidden_state):
        ...
        return hidden_state + shortcut


# Copied from transformers.models.resnet.modeling_resnet.ResNetBottleNeckLayer with ResNet->ResNetv2
...
class ResNetv2Stage(nn.Module):
    """
    A ResNetv2 stage composed by stacked layers.
    """
    ...
    def __init__(
        ...
        layer = ResNetv2PreActBottleneck if config.layer_type == "preactivation" else ResNetv2BottleNeckLayer
    ...
    # Copied from transformers.models.resnet.modeling_resnet.ResNetStage.forward with ResNet->ResNetv2
    def forward(self, input: Tensor) -> Tensor:
        hidden_state = input
...
# Copied from transformers.models.resnet.modeling_resnet.ResNetEncoder with ResNet->ResNetv2
class ResNetv2Encoder(nn.Module):
    def __init__(self, config: ResNetv2Config):
        ...
        for (in_channels, out_channels), depth in zip(in_out_channels, config.depths[1:]):
            self.stages.append(ResNetv2Stage(config, in_channels, out_channels, depth=depth))
...
# Copied from transformers.models.resnet.modeling_resnet.ResNetModel with RESNET->RESNETV2,ResNet->ResNetv2
...
@add_start_docstrings(
    """
    ResNetv2 backbone, to be used with frameworks like DETR and MaskFormer.
    """,
    RESNETV2_START_DOCSTRING,
)
# Copied from transformers.models.resnet.modeling_resnet.ResNetBackbone with RESNET->RESNETV2,ResNet->ResNetv2,resnet->resnetv2,microsoft/resnet-50->nandwalritik/resnetv2
class ResNetv2Backbone(ResNetv2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(self, pixel_values: Optional[torch.FloatTensor] = None) -> BackboneOutput:
        ...
        >>> processor = AutoImageProcessor.from_pretrained("microsoft/resnetv2-50")
        >>> model = AutoBackbone.from_pretrained("microsoft/resnetv2-50")
